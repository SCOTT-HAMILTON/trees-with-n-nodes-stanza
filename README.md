...
We know that we have to take in consideration all (actually not all but we will see that in the [Side note on optimization](#side-note-on-optimization)) **internal-trees**.
...
a number of **leaf-nodes** that will be attached to it, this number doesn't include the 
...
In the end we get a list of Combination-Action-List. I apologise for this
barbaric name but I didn't take the time to really think of a better one.
...
Actually, this example C-A-L above is a real C-A-L generated by the algorithm
when computing the trees with 8 nodes and 5 leaf-nodes.
...
A **minimal-tree** is an **internal-tree** but with **final** leaf-nodes
already attached to the L-nodes, we shouldn't forget them because the C-A-L
doesn't include these attachments.
Here is the **minimal-tree** corresponding to our **internal-tree** :
```
tree:
A
├── B
...
Here is another C-A-L for the same **internal-tree**: `([1 (1)] [2 (0)])`
...
that brought my intention. All **internal-trees** are not valid, this is because if the
**internal-tree** has more leaf-nodes than the number of final **leaf-nodes** (M), then it's
impossible to find any repartition of **final** leaf-nodes that will create a valid **final-tree**.

This problem becomes a huge issue when M stays small and N grows.
Because, what we are basically asking to the algorithm is to generate trees with
...
how quickly the number of BAITs grows:
...
I splitted it in two optimizations:
```
...
```
This affects the root-children count the loop starts with.
